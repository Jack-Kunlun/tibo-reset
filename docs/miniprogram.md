...
| 4 | 信号区 | 优先级链 `explicit → hint`。**已发生不在横幅上列举** —— 它的载体是首页顶部的「距上次重置 N 天」（见 `AGENTS.md` 的信号一节）。**醒目样式**（大字 + 光斑）只在拿到 `explicit` 时间窗口时出现；只有 `hint` 时**不得**出现醒目告警样式 |
...
| 10 | 信号**预告态**的视觉 | 真实数据长期触发不到这条路径。跑 `npm run preview:mp -- --demo-signal` 看一眼：大字星期、秒级倒计时（**锚点「北京时间 …」必须挨着数字**）、朱砂晕染、呼吸光斑是否都在 |
| 11 | 空闲态文案 | 必须是「时间窗内 N 条推文中没有检测到重置**预告**」。真实快照的 `signals` 里同时有 2 条 `occurred`，写成「没有检测到重置信号」就是假话 |
