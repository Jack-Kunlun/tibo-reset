...
| 4 | 信号区 | 分优先级链 `explicit → occurred → hint`。**醒目样式**（大字 + 光斑）只在拿到 `explicit` 时间窗口或 `occurred` 发生时刻时出现；只有 `hint`（额度意图但凑不齐语义）时**不得**出现醒目告警样式 |
...
| 11 | 信号**已发生态**的视觉 | 真实快照里 09-12 那次重置会走到这条路径。检查大字是**发生日的星期**、副行是「月日 + 时刻 + 距今多久」，且**没有**倒计时 |
