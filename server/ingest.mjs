 ...
 *
 * 为什么是这个方向（境外出、境内入）：
 *   两个硬约束把采集和服务分到了两个网络域 —— x.com 境内不通（采集必须在境外），
 *   小程序要求后端域名已备案（服务必须在境内）。谁主动连谁是有讲究的：
 *   境内访问 github.com 虽然通，但长期链路一抖就会出现「页面是新的、API 是旧的」
 *   这种最难排查的静默不一致。反过来 GitHub runner 连境内公网入口没有任何障碍。
 *
