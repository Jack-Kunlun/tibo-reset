...
export async function runCollection(opts = {}) {
  ...
  //
  // 它诞生于「CI 兜底采集」时代：那时采集有两处发起方 —— 本机（住宅出口，能过 Cloudflare）
  // 与 CI（机房 IP，必被挑战）。CI 若不看数据新鲜度照样去采，那次注定失败，会把
  // stats.json 的 errors 写成非空，于是本机刚清掉的「数据采集异常」横幅又被贴回页面上 ——
  // 明明数据是新鲜的，却告警说采不到。
  //
  // ⚠ 那个 CI 兜底 2026-09-22 已移除（见 docs/decisions.md D-023），所以**现在没有自动
  //   调用方**，能力留给手动重跑（`--max-age=<分钟>`）。若将来重新用于自动化，阈值必须
  //   **≥ 采集周期**，否则短路名存实亡：判据是「数据年龄 < skipIfFresherThanMs」，而数据
  //   年龄从刚采完的 0 涨到下一轮采集前的满一个周期 —— 阈值小于周期就意味着有一段窗口
  //   必然超阈值，于是每一轮都真去采、必然失败、必然写 errors。历史上这里正是
  //   120 < 480 这个配置矛盾，让 CI 有 75% 的运行在必然失败。
