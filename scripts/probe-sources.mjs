 ...
 * 而在本地是超时。**两个环境都不可能靠另一方推断**，只能在 runner 里实测。用法：
 *
 *   node scripts/probe-sources.mjs            # 全员探一遍
 *   node scripts/probe-sources.mjs syndication # 只探名字含该关键字的通道
 *
 * ⚠ 2026-09-22 更正一处归因：上面「本地超时」的原因**不是**「境内 DNS 污染」。
 *   实测本机经代理能直达 x.com（`curl -x http://127.0.0.1:7890 https://x.com/` → 200），
 *   采集走 CDP + 真实 Chrome（跟系统代理）也因此一直在成功。本文件超时的真正原因是
 *   **它用原生 fetch、不走代理**（原生 fetch 也不认 HTTPS_PROXY 环境变量，已实测）。
 *   所以本机的探针结果只说明「探针自己出不去」，不代表这条链路不通 ——
 *   这也正是采集能从本机跑、而 CI 跑不了的原因（Cloudflare 拦的是机房 IP 段）。
 *
